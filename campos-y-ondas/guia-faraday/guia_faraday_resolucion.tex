...
\usepackage{caption}
\usepackage{float}
\usetikzlibrary{arrows.meta,calc,patterns,babel,decorations.pathmorphing,decorations.pathreplacing,positioning}
...
\fancyhead[R]{Gu\'ia Faraday resuelta}
\fancyfoot[C]{\thepage}
...
\ideaclave{Este problema no requiere calcular la fem. La pregunta es s\'olo por el sentido de la corriente inducida. Por eso se debe identificar primero si el flujo externo aumenta o disminuye, y luego aplicar la ley de Lenz.}
...
\importante{La frase correcta no es ``la espira se opone al im\'an''. Lo correcto es: la espira se opone al cambio del flujo magn\'etico que atraviesa su superficie.}
...
\ideaclave{La bobina secundaria no est\'a conectada a la bater\'ia. Por tanto, cualquier corriente en la secundaria aparece por inducci\'on mutua. La pregunta central es si el flujo magn\'etico que atraviesa la bobina secundaria est\'a cambiando.}
...
\importante{Un campo magn\'etico constante no induce corriente. Lo que induce corriente es un flujo magn\'etico variable.}
...
\ideaclave{Aqu\'i el campo magn\'etico es uniforme y perpendicular a la espira. Por eso el flujo se escribe directamente como $\Phi_B=BA$. No hace falta integrar.}
...
\importante{El signo de Faraday se usa para fijar el sentido de la corriente inducida. Como este problema s\'olo pide la corriente resultante, basta entregar la magnitud.}
...
\ideaclave{Este problema \textbf{no se resuelve con $\Phi_B=BA$} porque el campo del alambre no es uniforme. Cerca del alambre el campo es mayor y lejos del alambre es menor. Por eso se integra el flujo sobre la superficie de la espira.}
...
\importante{Con los datos escritos en el enunciado, la fem queda del orden de microvoltios: $|\varepsilon|\approx4.8\,\mu\mathrm{V}$. Si una pauta entrega $4.8\,\mathrm{mV}$, probablemente hay una discrepancia en alguna unidad o dato num\'erico.}
